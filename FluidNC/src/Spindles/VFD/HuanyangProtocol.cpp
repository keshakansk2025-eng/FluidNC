    ...
    namespace VFD {
        ...
        void HuanyangProtocol::set_speed_command(uint32_t dev_speed, ModbusCommand& data) {
            ...
            if (dev_speed != 0 && (dev_speed < _minFrequency || dev_speed > _maxFrequency)) {
                log_warn("Huanyang requested freq " << (dev_speed) << " is outside of range (" << _minFrequency << "," << _maxFrequency
                                                    << ")");
            }
                ...
                case -2:
                    ...
                    return [](const uint8_t* response, VFDSpindle* vfd, VFDProtocol* detail) -> bool {
                        ...
                        log_info("Huanyang PD0011, PD005 Freq range ("
                                 << (huanyang->_minFrequency / 100) << "," << (huanyang->_maxFrequency / 100) << ") Hz"
                                 << " (" << (huanyang->_minFrequency / 100 * 60) << "," << (huanyang->_maxFrequency / 100 * 60) << ") RPM");
                ...
                case -4:
                    ...
                    return [](const uint8_t* response, VFDSpindle* vfd, VFDProtocol* detail) -> bool {
                        ...
                        if (value <= 4 && value >= 2) {
                            ...
                            log_info("Huanyang PD143 Poles:" << huanyang->_numberPoles);

                            huanyang->updateRPM(vfd);

                            return true;
                        } else {
                            log_error("Huanyang  PD143 Poles: expected 2-4, got:" << value);
                            return false;
                        }
                    };
                    break;
