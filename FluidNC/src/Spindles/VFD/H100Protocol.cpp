    ...
    namespace VFD {
        ...
        void H100Protocol::set_speed_command(uint32_t dev_speed, ModbusCommand& data) {
            ...
            if (dev_speed != 0 && (dev_speed < _minFrequency || dev_speed > _maxFrequency)) {
                log_warn("H100 requested freq " << (dev_speed) << " is outside of range (" << _minFrequency << "," << _maxFrequency << ")");
            }
        ...
        VFDProtocol::response_parser H100Protocol::initialization_sequence(int32_t index, ModbusCommand& data, VFDSpindle* vfd) {
            // NOTE: data length is excluding the CRC16 checksum.
            data.tx_length = 6;
            data.rx_length = 5;
