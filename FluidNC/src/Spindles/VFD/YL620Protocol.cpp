...
namespace Spindles {
    namespace VFD {
        ...
        VFDProtocol::response_parser YL620Protocol::initialization_sequence(int32_t index, ModbusCommand& data, VFDSpindle* vfd) {
            if (index == -1) {
                data.tx_length = 6;
                data.rx_length = 5;
                ...
                return [](const uint8_t* response, VFDSpindle* vfd, VFDProtocol* detail) -> bool {
                    ...
                    vfd->_slop = std::max(int(yl620->_maxFrequency) / 40, 1);
