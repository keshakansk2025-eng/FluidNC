...
namespace Machine {
    ...
    void IRAM_ATTR Axes::set_disable(int32_t axis, bool disable) {
    ...
    size_t Axes::findAxisIndex(const MotorDrivers::MotorDriver* const driver) {
        for (int32_t i = 0; i < _numberAxis; ++i) {
            for (int32_t j = 0; j < Axis::MAX_MOTORS_PER_AXIS; ++j) {
    ...
    Axes::~Axes() {
        for (int32_t i = 0; i < MAX_N_AXIS; ++i) {
            if (_axis[i] != nullptr) {
                delete _axis[i];
            }
        }
