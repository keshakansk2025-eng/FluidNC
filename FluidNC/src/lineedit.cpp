...
Lineedit::Lineedit(Print* _out, char* line, int32_t linelen) : out(_out), needs_reecho(false), startaddr(line), maxaddr(line + linelen) {
    restart();
}
...
bool Lineedit::already_in_history(const char* adr, int32_t len) {
    const char* first;
    const char* thischar;
    int32_t     i;
    if (!saved_length)
...
void Lineedit::add_to_history(const char* adr, int32_t len) {
    validate_history();
...
extern int32_t num_initial_matches(const char* key, int32_t keylen, int32_t matchnum, char* matchname);
...
void Lineedit::propose_word() {
    ...
    int32_t newmatchlen = strlen(name);
...
// cppcheck-suppress unusedFunction
int32_t Lineedit::finish() {
    int32_t length = (int)(endaddr - startaddr);
...
// cppcheck-suppress unusedFunction
bool Lineedit::realtime(int32_t c) {
    if (!editing) {
        return true;
    }
    if (escaping < 0 && c == '~') {
