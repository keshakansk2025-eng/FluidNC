...
class Lineedit {
private:
    static const int32_t MAXHISTORY = 400;
    Print*               out;
    ...
    bool already_in_history(const char* adr, int32_t len);
    void add_to_history(const char* adr, int32_t len);
...
public:
    Lineedit(Print* out, char* line, int32_t linelen);

    void    start(char* addr, int32_t count);
    int32_t finish();
    bool    step(int32_t c);
    bool    realtime(int32_t c);
};
