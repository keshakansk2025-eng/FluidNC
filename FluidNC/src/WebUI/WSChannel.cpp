...
namespace WebUI {
    ...
    WSChannel* WSChannels::getWSChannel(int32_t pageid) {
    ...
    bool WSChannels::runGCode(int32_t pageid, std::string_view cmd) {
    ...
    bool WSChannels::sendError(int32_t pageid, std::string err) {
        WSChannel* wsChannel = getWSChannel(pageid);
        if (wsChannel) {
