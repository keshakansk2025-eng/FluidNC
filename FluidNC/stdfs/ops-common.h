...
namespace std _GLIBCXX_VISIBILITY(default) {
    ...
    namespace filesystem {
        ...
        inline bool is_not_found_errno(int32_t err) noexcept { return err == ENOENT || err == ENOTDIR; }
...
#    endif
        ...
        bool do_copy_file(const char*                from,
                          const char*                to,
...
#ifdef _GLIBCXX_HAVE_SYS_STAT_H
    typedef struct ::stat stat_type;
    inline file_type      make_file_type(const stat_type& st) noexcept {
...
#    endif
    ...
    inline file_status make_file_status(const stat_type& st) noexcept {
        return file_status { make_file_type(st), static_cast<perms>(st.st_mode) & perms::mask };
    }
    inline std::filesystem::copy_options_existing_file copy_file_options(copy_options opt) {
        using std::filesystem::is_set;
        return { is_set(opt, copy_options::skip_existing),
