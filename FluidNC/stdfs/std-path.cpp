    ...
    if (_M_type == _Type::_Multi) {
        if (!_M_cmpts.empty()) {
            ...
            if (cmpt->_M_type == _Type::_Filename && !cmpt->empty()) {
                ...
                if (prev->_M_type == _Type::_Root_dir || prev->_M_type == _Type::_Root_name) {
                    _M_cmpts.erase(cmpt);
                    _M_trim();
                } else
                    cmpt->clear();
...
std::pair<const path::string_type*, std::size_t> path::_M_find_extension() const {
...
path::string_type path::_S_convert_loc(const char* __first, const char* __last, const std::locale& __loc) {
    ...
    if (!__str_codecvt_in(__first, __last, __ws, __cvt))
        _GLIBCXX_THROW_OR_ABORT(filesystem_error("Cannot convert character sequence", std::make_error_code(errc::illegal_byte_sequence)));
#    ifdef _GLIBCXX_FILESYSTEM_IS_WINDOWS
    return __ws;
#    else
...
namespace std {
    ...
    namespace filesystem {
        string fs_err_concat(const string& __what, const string& __path1, const string& __path2) {
            const size_t __len =
                18 + __what.length() + (__path1.length() ? __path1.length() + 3 : 0) + (__path2.length() ? __path2.length() + 3 : 0);
            string __ret;
            __ret.reserve(__len);
        ...
        std::string filesystem_error::_M_gen_what() { return fs_err_concat(system_error::what(), _M_path1.native(), _M_path2.native()); }
